...
object Day10 {
    fun solveA(input: List<String>): Int = computeCycles(input)
        .filter { it.cycle in (20..220 step 40) }
        .sumOf { it.registerX * it.cycle }

    fun solveB(input: List<String>): String = computeCycles(input)
        .windowed(step = 40, size = 40)
        .joinToString("\n", prefix = "\n") {
            it.joinToString("") {
                    state -> if ((state.cycle - 1)  % 40 in (state.registerX - 1..state.registerX + 1)) "#" else "."
            }
        }

    private fun computeCycles(input: List<String>) = input
